...
import { ClientPerProgram } from "@/components/ClientPerProgram";
import { getAllClientsPerProgram } from "@/services/clientsperprogram";
...
const page = () => {
  ...
  const getClientsRegister = () => {
    getAllClientsPerProgram().then((response) => {
      if (response.data) {
  ...
  const openClientProgramModal = () => {
    clientPerProgramEditModal.openModal(true);
  }
    ...
    <div className="w-full h-screen px-4">
      <div className="hidden md:flex justify-between items-center mb-20">
        <h1 className="hidden md:flex items-center gap-2 text-2xl font-semibold text-main-blue md:ml-2">
          Administração de Serviços - <span className="text-lg">Clientes por Programa</span>
        </h1>
        <div>
            ...
            >
              ...
              <div className="p-3 w-full flex justify-between align-center">
                <div className="flex flex-col gap-1">
                  <span className="md:font-bold w-64">{client.customerName}</span>
                  <span className="text-sm w-64">{client.programName}</span>
                </div>
                <div className="flex flex-col justify-end gap-5">
                  <span className="text-base font-semibold cursor-pointer">
                    <IoIosArrowForward
                      ...
                      onClick={() => {
                        if (auth && auth.setClientProgramId) {
                          auth.setClientProgramId(client.id);
                        }
                        openClientProgramModal()
                      }}
                    />
                  </span>
                </div>
      ...
      <Dialog
        open={clientPerProgramEditModal.isModalOpen}
        onOpenChange={(open) => {
          clientPerProgramEditModal.openModal(open);
          if (!open) getClientsRegister();
        }}
      ><ClientPerProgram /></Dialog>
    </div>
  );
};
